/* =========================================================
   Ruchi Bites — Notification helpers
...
async function sendEmail({ to, subject, html, text }){
  ...
  try {
    ...
    const info = await transport.sendMail({ from: `"Ruchi Bites" <${from}>`, to, subject, html, text });
...
async function notifyOrderSuccess({ shipping, items, total, paymentId, orderId }){
  ...
  const ownerMsg =
    `🔔 *New Order Received* — Ruchi Bites\n\n` +
    `${orderLinesText(items)}\n\n` +
  ...
  const customerEmailHtml = `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;color:#2b1a10;">
      <h2 style="color:#ea580c;">🥟 Ruchi Bites — Order Confirmed!</h2>
      ...
      <p style="color:#8a7461;font-size:12px;margin-top:24px;">Ruchi Bites · hello@ruchibites.com</p>
    </div>`;
  ...
  const results = await Promise.allSettled([
    ...
    sendEmail({ to: shipping.email, subject: "Your Ruchi Bites order is confirmed! 🥟", html: customerEmailHtml, text: customerMsg }),
